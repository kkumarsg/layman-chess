...
public class PossiblePathWhiteSoldierHelper {

    public static List<Postition> possiblePathForWhiteSoldier(Box[][] board, int x, int y) throws ChessGameException {
        List<Postition> possiblePositions = new ArrayList<>();
        if (x == 7) {
            return possiblePositions;
        }
        if (x <= 0) {
            throw new ChessGameException("Illegal starting point for White Soldier ");
        }
        if (board[x + 1][y] == null) {
            possiblePositions.add(new Postition(x + 1, y));
        }
        if (y > 0 && board[x + 1][y - 1] == null || board[x + 1][y - 1].getPawn() == null ||
                PawnType.BLACK.equals(board[x + 1][y - 1].getPawn().getType())) {
